...
model = pickle.load(open(r'C:\xampp\htdocs\code\fertilizer.pkl', 'rb'))
...
app.config['MAIL_USERNAME']='madhurikadam1780@gmail.com'
app.config['MAIL_PASSWORD']='Madhuri1104@'
mail = Mail(app) 
...
def home_page():
    ...
    return render_template('index.html', data01=data01[0].get("Users"), data02=data02[0].get("Predictions"),
                           data03=data03[0].get("Locations"), data04=data04[0].get("Plots"))
...
def forgot_password():
    if request.method == 'POST':
        ...
        cur=mysql.connection.cursor()
        result=cur.execute("SELECT * FROM admin_details WHERE Email_Address=%s",[email])
        if(result>0):
            data=cur.fetchone()
            msg=Message(subject="Forgot Password Request",sender="madhurikadam1780@gmail.com",recipients=["madhurikadam220@gmail.com"])
            msg.body=render_template('sent.html',data=data)
            mail.send(msg)
            
            flash("Email already sent to your email address.",'success')
...
def update_admin():
    ...
    fe=request.form['fe']
    fp=request.form['fp']
    cur.execute("SELECT * FROM admin_details WHERE Email_Address LIKE '{}'and Password like '{}' ".format(fe,fp))
